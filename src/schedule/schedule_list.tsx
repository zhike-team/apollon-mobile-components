    ...
    return (
      <div className='schedule-list'>
        <header>
          ...
          <span className='status'>{status === 'FINISHED' ? <Icon type='correct-thick' size='sm' /> : status === 'UNFINISHED' ? <Icon type='cross-red' size='xs' /> : <Icon type='question' size='xs' />}{status === 'FINISHED' ? <span className='blue'>已完成</span> : status === 'UNFINISHED' ? <span className='red'>未完成</span> : <span className='gray'>未确认</span>}</span>
        </header>
        <main>
          <div className='name'>
